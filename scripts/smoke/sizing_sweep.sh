...
#BSUB -o /lustre/scratch126/cellgen/lotfollahi/ha11/bd3lms/logs/sizing_sweep_%J.out
#BSUB -e /lustre/scratch126/cellgen/lotfollahi/ha11/bd3lms/logs/sizing_sweep_%J.err
set -euo pipefail
...
"$PYTHON" -u scripts/smoke/sizing_sweep.py \
  ...
  --checkpoint-modes "$CHECKPOINT_MODES" \
  --warmup "$WARMUP" \
  --iters "$ITERS" \
  --output "$OUTPUT"
